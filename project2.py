...
def signup():
    ...
    try:
        ...
        # Log saved
        save_logs("Log: Sign-up Successful")
            ...
            if row[0] == email:
                ...
                if bcrypt.checkpw(password.encode('utf-8'), row[1].encode('utf-8')):
                    ...
                    with open(LOGS_FILE, mode="a", newline='') as f:
                        f.write("Log: Successfully Logged In By: ")
                        f.write(email)
                        f.write("\n")
                    return True
                else:
...
def reset_password():
    ...
    try:
        with open(USER_DATA_FILE, mode='r') as file:
            ...
            for row in users:
                if row[0] == email:
                    security_answer = input("What is your best friend's name? ")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            if login():
                while True:
                    ticker_symbol = input("Enter stock ticker symbol (e.g., ITC): ").upper()
